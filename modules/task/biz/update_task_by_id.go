package biz

import (
	"context"
	"errors"
	"iTask/common"
	"iTask/modules/task/model"
)

type UpdateTaskStorage interface {
	GetTask(ctx context.Context, cond map[string]interface{}, moreKeys ...string) (*model.Task, error)
	UpdateTask(ctx context.Context, cond map[string]interface{}, dataUpdate *model.TaskUpdate) error
}

type updateTaskBiz struct {
	store     UpdateTaskStorage
	requester common.Requester
}

func NewUpdateTaskBiz(store UpdateTaskStorage, requester common.Requester) *updateTaskBiz {
	return &updateTaskBiz{store: store, requester: requester}
}

func (biz *updateTaskBiz) UpdateTask(ctx context.Context, id int, dataUpdate *model.TaskUpdate) error {
	data, err := biz.store.GetTask(ctx, map[string]interface{}{"id": id})

	if err != nil {
		return err
	}

	if data.Status.String() == "Deleted" {
		return common.ErrEntityDeleted(model.EntityName)
	}

	isOwner := data.CreatedBy == biz.requester.GetUserId()

	if !isOwner && !common.IsAdmin(biz.requester) {
		return common.ErrNoPermission(errors.New("you don't have permission to update this item"))
	}

	if err := biz.store.UpdateTask(ctx, map[string]interface{}{"id": id}, dataUpdate); err != nil {
		return common.ErrCannotUpdateEntity(model.EntityName, err)
	}

	return nil
}
